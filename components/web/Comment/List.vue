<template>
    <div class="card">
        <header class="card-header">
            <p class="card-header-title title is-5">Komentar</p>
        </header>
        <div class="card-content pt-2">
            <form>
                ...
                <div v-if="isAuthenticated">
                    <keep-alive>
                        <comment-form @valueUpdated="updateComment($event)" />
                    </keep-alive>
                </div>
                <template v-if="comments.length < 1">
                    <div class="has-text-centered">Belum ada komentar</div>
                </template>
                ...
                <article class="media" v-for="data in comments" :key="data.id">
                    <figure class="media-left">
                        ...
                        >
                            <nuxt-img
                                :src="`/storage/users/` + data.user.image"
                                :alt="data.user.name"
                            />
                        </p>
                        <p v-else class="image is-64x64">
                            <img
                                src="https://b.kisscc0.com/20180718/urw/kisscc0-ninja-computer-icons-samurai-youtube-avatar-ninja-5b4ed903c2dd20.4931332915318940197982.jpg"
                            />
                        </p>
                    </figure>
                    <div class="media-content">
                        <div class="content">
                            <p>
                                ...
                                <br />
                                <small
                                    ><a @click.prevent="replyBox(data.id)"
                                        >Reply</a
                                    >
                        ...
                        >
                            <figure class="media-left">
                                ...
                                >
                                    <nuxt-img
                                        :src="
                                            `/storage/users/` + reply.user.image
                                        "
                                        :alt="reply.user.name"
                                    />
                                </p>
...
export default {
    ...
    methods: {
        ...
        updateReply({ value, parentId }) {
            ...
            this.$store.dispatch('web/post/updateReply', {
                comment: value,
                parentId,
            })
        },
